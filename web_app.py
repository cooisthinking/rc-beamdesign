...
with col2:
    ...
    # 🌟 ฟีเจอร์ใหม่: เลือกชั้นคุณภาพเหล็กเสริม (Fy)
    ...
    if fy_sel == "กำหนดเอง":
        ...
        # ดึงตัวเลขออกมาจากข้อความใน Dropdown (เช่น 4000)
...
with col4:
    ...
    # 🌟 ฟีเจอร์ใหม่: เพิ่มเหล็ก RB ลงในตัวเลือก
    ...
    if rebar_m_pos == "กำหนดขนาดเอง":
        dia_m_pos = st.number_input("เส้นผ่านศูนย์กลาง (mm) M+", min_value=6.0, value=12.0)
    else:
        dia_m_pos = float(rebar_m_pos.replace("DB", "").replace("RB", ""))
        
    n_m_pos = st.number_input("จำนวนเส้น M+", min_value=1, value=2, step=1)
    
    # 🌟 ฟีเจอร์ใหม่: ระบบเตือนถ้าใช้เหล็ก RB แต่ไม่ได้เลือก SR24
    is_rb_pos = "RB" in rebar_m_pos or (rebar_m_pos == "กำหนดขนาดเอง" and dia_m_pos < 10)
    if is_rb_pos and F != 2400:
        st.warning("⚠️ **แจ้งเตือน:** คุณกำลังเลือกใช้เหล็กเส้นกลม (RB) โปรดเลื่อนขึ้นไปเลือกชั้นคุณภาพเหล็กเป็น **'SR24 (fy=2400)'** ที่หัวข้อที่ 1 เพื่อให้รายการคำนวณถูกต้องครับ!")

    AStu = n_m_pos * (math.pi * (dia_m_pos/10)**2 / 4)
    st.info(f"**พื้นที่เหล็กที่จัดให้ (As Provided):** {AStu:.2f} sq.cm")
...
if calc_m_neg:
    ...
    with col6:
        ...
        if rebar_m_neg == "กำหนดขนาดเอง":
            dia_m_neg = st.number_input("เส้นผ่านศูนย์กลาง (mm) M-", min_value=6.0, value=12.0)
        else:
            dia_m_neg = float(rebar_m_neg.replace("DB", "").replace("RB", ""))
        ...
        st.info(f"**พื้นที่เหล็กที่จัดให้ (As Provided):** {AStu_neg:.2f} sq.cm")
